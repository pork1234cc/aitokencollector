...
- 圆形透明悬浮窗，外环按 Claude Code / Codex 用量占比分色，圆心只显示含缓存总数，悬浮时显示不含缓存读取及分类明细
- 支持深色、浅色、霓虹、极简四套主题
- 拖动定位、透明度调节（20%～100%）、置顶、锁定位置，重启后自动恢复
- 贴边自动隐藏：拖到屏幕边缘（20px 内）松手即吸附，鼠标离开 1.5 秒后滑出只留 6px 细边，移回细边自动滑回
...
- 无强制第三方依赖；托盘功能需要 `pystray` + `pillow`，建议装到项目本地虚拟环境（见下）

## 快速开始

### 方式一：双击启动（推荐，无需打开 VSCode）

双击项目根目录下的 `启动.bat`，会用 `pythonw` 静默启动悬浮窗（不弹黑色命令行窗口）。若存在项目本地虚拟环境 `.venv`，脚本会优先使用其中的 `pythonw`，否则回退到系统 `pythonw`。

### 托盘功能依赖（可选，装到项目本地虚拟环境）
...
之后无论用 `启动.bat` 还是 `.venv\Scripts\python.exe token_stats.py` 启动，右键菜单的"隐藏到托盘"都会生效。`.venv` 已加入 `.gitignore`，不会被提交。
...
- **展示口径**：含缓存总数包含缓存读取；不含缓存读取总数等于含缓存总数减去缓存读取。Codex 的缓存是输入子集、推理是输出子集，明细展示时会先转换为互斥成分，避免重复相加。
- 日期归属按事件时间戳的本地时区日期计算。
